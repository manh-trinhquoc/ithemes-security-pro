...
class Settings extends \WP_REST_Controller {
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! $this->source->has( $request['id'] ) ) {
			return new \WP_Error( 'rest_user_group_not_found', __( 'No user group found.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::NOT_FOUND ] );
		}
		...
		foreach ( $matchables as $matchable ) {
			...
			try {
				...
				$return[] = [
					...
					'status' => \WP_Http::NOT_FOUND,
					'error'  => rest_get_server()->response_to_data( \ITSEC_Lib_REST::error_to_response(
						new \WP_Error( 'internal_server_error', __( 'An unexpected error occurred.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::INTERNAL_SERVER_ERROR ] )
					), false ),
				];
			}
		}
