...
class ITSEC_Sync_Connect {
	...
	public function install_plugin() {
		...
		if ( true !== $installed && ! is_wp_error( $installed ) ) {
			...
			if ( ! $error->has_errors() ) {
				$error->add( 'installation_failed_unknown_reason', __( 'Installation failed for an unknown reason.', 'it-l10n-ithemes-security-pro' ) );
			}
	...
	public function send_activation_request( $username, $token, $sync_nonce ) {
		...
		if ( $request['response']['code'] !== 200 ) {
			return new WP_Error( 'itsec-sync-connect-invalid-response', sprintf( __( 'Invalid response from the server (Code: %d). Please manually activate the plugin.', 'it-l10n-ithemes-security-pro' ), $request['response']['code'] ) );
		}

		$response_body = json_decode( wp_remote_retrieve_body( $request ), true );

		if ( ! $response_body ) {
			return new WP_Error( 'itsec-sync-connect-invalid-json', __( 'Invalid JSON response from Sync API. Please manually activate the plugin.', 'it-l10n-ithemes-security-pro' ) );
		}
