...
class ITSEC_Settings_Page_Sidebar_Widget_Active_Lockouts extends ITSEC_Settings_Page_Sidebar_Widget {
	...
	public function render( $form ) {
		...
		if ( ! empty( $users ) ) {
			echo '<p><strong>' . __( 'Users', 'it-l10n-ithemes-security-pro' ) . "</strong></p>\n";
			echo "<ul>\n";
			...
			foreach ( $users as $user_id => $data ) {
				...
				if ( $user ) {
					...
					$label = sprintf( __( 'Deleted #%d', 'it-l10n-ithemes-security-pro' ), $user_id );
				}

				/* translators: 1. Username 2. Expiration as human time diff */
				$label = sprintf( _x( '%1$s - Expires in %2$s', 'User lockout', 'it-l10n-ithemes-security-pro' ), "<strong>{$label}</strong>", '<em>' . human_time_diff( $data[1] ) . '</em>' );
				echo '<li><label>';
		...
		if ( ! empty( $usernames ) ) {
			echo '<p><strong>' . __( 'Usernames', 'it-l10n-ithemes-security-pro' ) . "</strong></p>\n";
			echo "<ul>\n";

			foreach ( $usernames as $username => $data ) {
				/* translators: 1. Username 2. Expiration as human time diff */
				$label = sprintf( _x( '%1$s - Expires in %2$s', 'Username lockout', 'it-l10n-ithemes-security-pro' ), '<strong>' . esc_html( $username ) . '</strong>', '<em>' . human_time_diff( $data[1] ) . '</em>' );
				echo '<li><label>';
		...
		if ( ! empty( $hosts ) ) {
			echo '<p><strong>' . __( 'Hosts', 'it-l10n-ithemes-security-pro' ) . "</strong></p>\n";
			echo "<ul>\n";

			foreach ( $hosts as $host => $data ) {
				/* translators: 1. IP Address 2. Expiration as human time diff */
				$label = sprintf( _x( '%1$s - Expires in %2$s', 'Host lockout', 'it-l10n-ithemes-security-pro' ), '<strong>' . esc_html( strtoupper( $host ) ) . '</strong>', '<em>' . human_time_diff( $data[1] ) . '</em>' );
				echo '<li><label>';
	...
	protected function save( $data ) {
		...
		if ( ! empty( $data['users'] ) && is_array( $data['users'] ) ) {
			foreach ( $data['users'] as $id ) {
				...
				if ( ! $result ) {
					$this->errors[] = sprintf( __( 'An unknown error prevented releasing the user lockout with a lockout ID of %d', 'it-l10n-ithemes-security-pro' ), $id );
				}
			}
		}
		...
		if ( ! empty( $data['usernames'] ) && is_array( $data['usernames'] ) ) {
			foreach ( $data['usernames'] as $id ) {
				...
				if ( ! $result ) {
					$this->errors[] = sprintf( __( 'An unknown error prevented releasing the username lockout with a lockout ID of %d', 'it-l10n-ithemes-security-pro' ), $id );
				}
			}
		}
		...
		if ( ! empty( $data['hosts'] ) && is_array( $data['hosts'] ) ) {
			foreach ( $data['hosts'] as $id ) {
				...
				if ( ! $result ) {
					$this->errors[] = sprintf( __( 'An unknown error prevented releasing the host lockout with a lockout ID of %d', 'it-l10n-ithemes-security-pro' ), $id );
				}
			}
		}
		...
		if ( empty( $this->errors ) ) {
			if ( $count > 0 ) {
				$this->messages[] = _n( 'Successfully removed the selected lockout.', 'Successfully remove the selected lockouts.', $count, 'it-l10n-ithemes-security-pro' );
			} else {
