...
class ITSEC_Lib_Static_Map_API {
	...
	public static function get_map( $config ) {
		...
		if ( ! is_array( $config ) || ! isset( $config['lat'], $config['long'] ) || ! is_numeric( $config['lat'] ) || ! is_numeric( $config['long'] ) ) {
			return new WP_Error( 'itsec-static-map-api-invalid-config', __( 'Invalid configuration for retrieving a static map image.', 'it-l10n-ithemes-security-pro' ) );
		}

		if ( ! self::get_api() ) {
			return new WP_Error( 'itsec-static-map-api-no-providers', __( 'No provider was found to generate a static map image.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	private static function get_resized_map( $file, array $config ) {
		...
		if ( $width > 1000 || $height > 1000 ) {
			return new WP_Error( 'itsec-static-map-api-invalid-resize-dimensions', __( 'Maximum map dimensions is 1000px.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	private static function get_cached_map_or_fetch( array $config ) {
		...
		if ( ! file_exists( $dir . $file_name ) ) {
			...
			if ( ! function_exists( 'download_url' ) ) {
				return new WP_Error( 'itsec-static-map-api-file-cache-no-download-url', __( 'The download_url() function was not found.', 'it-l10n-ithemes-security-pro' ) );
			}
