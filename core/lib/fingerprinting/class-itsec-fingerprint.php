...
class ITSEC_Fingerprint implements JsonSerializable {
	...
	public function __toString() {
		...
		if ( $location && $browser ) {
			$str = sprintf( esc_html__( 'Device running %1$s on %2$s near %3$s', 'it-l10n-ithemes-security-pro' ), $browser, $platform, $location );
		} elseif ( $location ) {
			$str = sprintf( esc_html__( 'Device near %1$s', 'it-l10n-ithemes-security-pro' ), $location );
		} elseif ( $browser ) {
			$str = sprintf( esc_html__( 'Device running %1$s on %2$s', 'it-l10n-ithemes-security-pro' ), $browser, $platform );
		} else {
			$str = '';
		}
